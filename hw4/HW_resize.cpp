...
HW_resize(ImagePtr I1, int ww, int hh, int kernelID, double param, ImagePtr I2)
{
	...
	int t, j, k, offset;
	ChannelPtr<uchar> src, dst, temp;
	for (int ch = 0; IP_getChannel(II, ch, src, t); ch++) {
		IP_getChannel(I2, ch, dst, t);
		IP_getChannel(I3, ch, temp, t);


		offset = 1;
		j = k = 0;
		// horizontal pass scales rows
		for (int y = 0; y < h; ++y, j += w, k += ww) {
			resize1D(src + j, w, ww, offset, kernelID, param, temp + k);
		}

		offset = ww;
		// vertical pass scales columns
		for (int x = 0; x < ww; ++x) {
			resize1D(temp + x, h, hh, offset, kernelID, param, dst + x);
		}
	}
